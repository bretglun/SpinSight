...
PARAM_VALUES = {
    ...
    'phase_oversampling': {str(int(po)) + '%': float(po) for po in range(0, (MAX_PHASE_OVERSAMPLING_FACTOR - 1) * 100 + 1)},
    'pixel_bandwidth_ui': {bw + ' Hz': float(bw) for bw in [formatting.format_float(bw, 3) for bw in bw_vals]},
    ...
    'matrix_P_ui': list(range(MIN_MATRIX, MAX_MATRIX+1)),
    'matrix_F_ui': list(range(MIN_MATRIX, MAX_MATRIX+1)),
    ...
    'recon_matrix_P_ui': list(range(MIN_RECON_MATRIX, MAX_RECON_MATRIX+1)),
    'recon_matrix_F_ui': list(range(MIN_RECON_MATRIX, MAX_RECON_MATRIX+1)),
